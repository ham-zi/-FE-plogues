...
function BoardForm() {
  ...
  const recommend = () => {
    api
      ...
      .catch((err) => {
        console.error(err);
        customAlert.error("제목 추천에 실패했습니다.");
      });
  };
  ...
  return (
    <FormWrap>
      ...
      <FormRow>
        ...
        <button type="button" className="reset-btn" onClick={recommend}>
          제목 추천받기
        </button>
        <input
          type="text"
      ...
      <FormButtons>
        ...
        <button
          type="button"
          className="cancel"
          onClick={() => navigate(isEdit ? `/reviews/${boardNo}` : "/reviews")}
        >
          취소
        </button>
      </FormButtons>
