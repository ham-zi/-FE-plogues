...
console.log('board.updated:', board.updated);
  ...
  return (
    <DetailWrap>
      ...
      {board.fileList && board.fileList.map((file) => (
        <DetailImage
          key={file.fileNo}
  ...
  {myUserId ? (
    ...
    <CommentLoginNotice>댓글 작성은 로그인 후 이용하실 수 있습니다.</CommentLoginNotice>
  )}
  ...
  {board.commentList && board.commentList.length > 0 ? (
    board.commentList.map((comment) => (
      <div key={comment.commentNo}>
        <CommentItem>
  <div className="profile" style={{ backgroundColor: getProfileColor(comment.commentNo) }} />
  <div className="body">
    <span className="writer">{comment.writer}</span>
    <span className="date">{formatDate(comment.createDate)}</span>
    {comment.updated === 'Y' && <span className="date"> (수정됨)</span>}
  ...
  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
    {comment.userId === myUserId && (
      <div className="actions">
        <button className="icon-btn" onClick={() => handleEditStart(comment)}>
          <FaEdit />
        </button>
        <button className="icon-btn" onClick={() => handleCommentDelete(comment.commentNo)}>
          <FaTrash />
        </button>
      </div>
    )}
